...
class DegreeCourse(models.Model):
    ...
    prerequisite = models.ManyToManyField("Teacher", verbose_name="课程讲师")
    # 用于GenericForeignKey反向查询，不会生成表字段们切勿删除
...
class Scholarship(models.Model):
    ...
    time_percent = models.PositiveSmallIntegerField(verbose_name="奖励档位(时间百分比)", help_text="只填百分比，如80，代表80%")
...
class DegreeCourseReview(models.Model):
    """学位课程评价
    为了以后可以定制单独的评价内容，所以不与普通课程的评价混在一起
    """
...
# 31
class HomewordRecord(models.Model):
    """学员作业记录及成绩"""
    homework = models.ForeignKey("Homework")
...
# 32
class StuFollowUpRdcord(models.Model):
    """学员跟进记录"""
    """学员跟进记录"""
